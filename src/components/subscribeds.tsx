...
import SubscribedsList from "./subscribedsList";

export default function Subscribeds() {
  const { setShowMessage, showSubscribeds, setShowSubscribeds } = useContext(contexto);
  ...
  useEffect(() => {
    const getActivities = async () => {
      const getSubs = await getSubscribedsByEvent(showSubscribeds.id, setShowMessage);
      if (getSubs) setListSubscribeds(getSubs);
      const allActivities = await getActivitiesByEventId(showSubscribeds.id, setShowMessage);
      ...
      <div className="break-words w-full lex flex-col justify-center items-center min-h-screen relative border-white border-2 mx-1 pb-5 mt-10">
        <div className="break-words pt-4 sm:pt-2 px-2 w-full flex justify-end top-0 right-0">
          <IoIosCloseCircleOutline
            ...
            onClick={() => setShowSubscribeds({ show: false, id: '' }) }
          />
        </div>
